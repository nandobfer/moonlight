...
-- Sorcerer Teleport
local portalKeyword = keywordHandler:addKeyword({'portal'}, StdModule.say, {npcHandler = npcHandler,
		text = {
			'Posso te ensinar o feitico de teleporte pra Liberty Bay. Tem interesse?',
		}}, function(player) return player:getVocation():getId() == 1 end
	)
	portalKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler,
		text = 'Eh so conjurar as palavras magicas *exani tele libertybay*.',
		reset = true, ungreet = true}, nil,
		function(player) player:setStorageValue(Storage_.teleport.libertybay, 1) end)
