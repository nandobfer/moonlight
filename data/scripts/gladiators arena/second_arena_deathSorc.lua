local tilePosition = {x = 32104, y = 32343, z = 7}
...
if item.uid == 65529 or item.uid == 65530 then 
...
--Salva vocação atual na storage / altera vocação para evil sorcerer
...
--Salva Outfit config nas storages / Seta Outifit evil sorcerer
    ...
    player:setOutfit({lookType = 130, lookHead = 0, lookBody = 114, lookLegs = 114, lookFeet = 114, lookAddons = 3})   
...
player:addItem(8094)--(Wand of Vodoo)
...
if Game.getStorageValue(100000024) < 1 then
    Game.setStorageValue(100000024, 1) --storage da arena
    player:teleportTo(Position(32087, 32339, 7))
    else 
    Game.setStorageValue(100000024, 0) --storage da arena
        player:teleportTo(Position(32087, 32341, 7))
end


player:setStorageValue(100000025, 1) --seta death_storage (cpp)
player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You became an Evil Sorcerer, your new spells are: "Exuras", "Utani Hurs", "Exori Morts", "Exevo Mort Hur", "Exori Max Mort".')
